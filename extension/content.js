...
async function analyzePRAutomatically() {
  ...
  try {
    const titleInput = document.querySelector('input#pull_request_title');
    const bodyInput = document.querySelector('textarea#pull_request_body');

    const prTitle = titleInput ? titleInput.value : '';
    const prBody = bodyInput ? bodyInput.value : '';
    ...
    chrome.storage.local.get(['fm_token'], async (result) => {
      ...
      try {
        ...
        if (response.status === 401) {
          // Try to refresh token via background script
